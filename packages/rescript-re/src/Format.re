...
type pp_token =
  ...
  | Pp_begin(int, box_type) /* beginning of a box */
  | Pp_end /* end of a box */
  | Pp_tbegin(tbox) /* beginning of a tabulation box */
  | Pp_tend /* end of a tabulation box */
  | Pp_newline /* to force a newline inside a box */
  | Pp_if_newline /* to do something only if this very
                                  line has been broken */
  | Pp_open_tag(stag) /* opening a tag name */
...
let format_string = (state, s) =>
  if (s != "") {
    format_pp_text(state, String.length(s), s);
  };
...
let break_line = (state, width) =>
  break_new_line(state, ("", 0, ""), width);
...
let pp_force_break_line = state =>
  switch (Stack.top_opt(state.pp_format_stack)) {
...
let pp_skip_token = state =>
  switch (Queue.take_opt(state.pp_queue)) {
...
let format_pp_token = (state, size) =>
  ...
  |  Pp_begin(off, ty) => {
      ...
      let width = state.pp_space_left - off;
      let box_type =
        switch (ty) {
        | Pp_vbox => Pp_vbox
        | Pp_hbox
  ...
  | Pp_stab =>
    switch (Stack.top_opt(state.pp_tbox_stack)) {
  ...
  | Pp_close_tag =>
    switch (Stack.pop_opt(state.pp_mark_stack)) {
...
let enqueue_string_as = (state, size, s) =>
  enqueue_advance(
    state,
    {size, token: Pp_text(s), length: Size.to_int(size)},
  );
...
let initialize_scan_stack = stack => {
  Stack.clear(stack);
  let queue_elem = {size: Size.unknown, token: Pp_text(""), length: 0};
...
let scan_push = (state, b, token) => {
  ...
  let elem = {left_total: state.pp_right_total, queue_elem: token};
...
let pp_open_box_gen = (state, indent, br_ty) => {
  ...
  if (state.pp_curr_depth < state.pp_max_boxes) {
    let size = Size.of_int(- state.pp_right_total);
    let elem = {
      size,
...
let pp_open_stag = (state, tag_name) => {
  ...
  if (state.pp_mark_tags) {
    let token = Pp_open_tag(tag_name);
...
let pp_close_stag = (state, ()) => {
  ...
  if (state.pp_print_tags) {
    switch (Stack.pop_opt(state.pp_tag_stack)) {
    | None => () /* No more tag to close. */
...
let clear_tag_stack = state =>
  Stack.iter(_ => pp_close_tag(state, ()), state.pp_tag_stack);
...
let pp_print_int = (state, i) => pp_print_string(state, Belt.Int.toString(i));
...
let pp_print_custom_break = (state, ~fits, ~breaks) => {
  ...
  if (state.pp_curr_depth < state.pp_max_boxes) {
    let size = Size.of_int(- state.pp_right_total);
    let token = Pp_break({fits, breaks});
    let length = String.length(before) + width + String.length(after);
    let elem = {size, token, length};
...
let pp_open_tbox = (state, ()) => {
  ...
  if (state.pp_curr_depth < state.pp_max_boxes) {
    let size = Size.zero;
    let elem = {size, token: Pp_tbegin(Pp_tbox(ref([]))), length: 0};
...
let pp_close_tbox = (state, ()) =>
  if (state.pp_curr_depth > 1) {
    if (state.pp_curr_depth < state.pp_max_boxes) {
      let elem = {size: Size.zero, token: Pp_tend, length: 0};
      enqueue_advance(state, elem);
      state.pp_curr_depth = state.pp_curr_depth - 1;
    };
  };
...
let pp_set_tab = (state, ()) =>
  if (state.pp_curr_depth < state.pp_max_boxes) {
    let elem = {size: Size.zero, token: Pp_stab, length: 0};
    enqueue_advance(state, elem);
  };
...
let check_geometry = geometry =>
  switch (validate_geometry(geometry)) {
  | Ok () => true
  | Error(_) => false
  };
...
let pp_set_geometry = (state, ~max_indent, ~margin) => {
  let geometry = {max_indent, margin};
  switch (validate_geometry(geometry)) {
...
let pp_get_geometry = (state, ()) => {
  margin: pp_get_margin(state, ()),
  max_indent: pp_get_max_indent(state, ()),
};

let pp_update_geometry = (state, update) => {
  let geometry = pp_get_geometry(state, ());
...
let make_formatter = (output, flush) => {
  let ppf = pp_make_formatter(output, flush, ignore, ignore, ignore);
...
let formatter_of_out_channel = oc =>
  make_formatter(output_substring(oc), () => flush(oc));
...
let flush_symbolic_output_buffer = sob => {
  let items = get_symbolic_output_buffer(sob);
...
let pp_print_option = (~none=(_, ()) => (), pp_v, ppf) =>
  fun
  | None => none(ppf, ())
  | Some(v) => pp_v(ppf, v);
...
let rec output_acc = (ppf, acc) =>
  ...
  |
    Acc_string_literal(
      ...
      Acc_formatting_lit(p,  Magic_size(_, size)),
      s,
    )
  |
    Acc_data_string(
  ...
  |  Acc_formatting_gen(p, Acc_open_box(acc')) =>
    output_acc(ppf, p);
    let (indent, bty) = open_box_of_string(compute_tag(output_acc, acc'));
...
let rec strput_acc = (ppf, acc) =>
  ...
  |  Acc_formatting_gen(p, Acc_open_box(acc')) =>
    strput_acc(ppf, p);
    let (indent, bty) = open_box_of_string(compute_tag(strput_acc, acc'));
    pp_open_box_gen(ppf, indent, bty);
  |  Acc_string_literal(p, s)
  |  Acc_data_string(p, s) =>
    strput_acc(ppf, p);
...
let fprintf = ppf => kfprintf(_, ppf);
let printf = fmt => fprintf(std_formatter, fmt);
let eprintf = fmt => fprintf(err_formatter, fmt);

let kdprintf = (k,  fmt) =>
  make_printf(acc => k(ppf => output_acc(ppf, acc)), End_of_acc, fmt);

// let dprintf = fmt => kdprintf(i => i, fmt);

let ksprintf = (k,  fmt) => {
  let b = pp_make_buffer();
...
let sprintf = fmt => ksprintf(_, fmt);
...
// let bprintf =
//     (b,  Format(fmt, _): format('a, formatter, unit)) => {
//   let ppf = formatter_of_buffer(b);
//   let k = acc => {
//     output_acc(ppf, acc);
//     pp_flush_queue(ppf, false);
...
let pp_get_formatter_tag_functions = (fmt, ()) => {
  let funs = pp_get_formatter_stag_functions(fmt, ());
  let mark_open_tag = s => funs.mark_open_stag(String_tag(s));
  let mark_close_tag = s => funs.mark_close_stag(String_tag(s));
  let print_open_tag = s => funs.print_open_stag(String_tag(s));
  let print_close_tag = s => funs.print_close_stag(String_tag(s));
